...
class WorkerExtractorTestCase(unittest.TestCase):
    ...
    def test_that_creating_extractor_works(self):
        ...
        settings = Settings()
        settings.SPYDER_EXTRACTOR_PIPELINE = ['spyder.processor.limiter',]
        ...
        master_push.bind(settings.ZEROMQ_WORKER_PROC_FETCHER_PUSH)

        time.sleep(.5)

        extractor = workerprocess.create_worker_extractor(settings, mgmt, ctx,
                io_loop)
        extractor.start()

        time.sleep(.5)

        master_pull = ctx.socket(zmq.PULL)
        master_pull.connect(settings.ZEROMQ_WORKER_PROC_SCOPER_PULL)
